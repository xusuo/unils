...
export default {
  ...
  data() {
    return {
      ...
      accept:
        "application/msword,application/vnd.ms-powerpoint,application/x-zip-compressed,application/x-rar-compressed,application/vnd.ms-excel",
      // 是否正在上传图片
      isUploading: false,
      // 上传的进度
      uploadProgress: 0,
      ...
      return new Promise((resolve, reject) => {
        this.$nextTick(() => {
          ...
          if (file.size / 1024 <= this.size) {
            ...
            if (this.size > 1024) {
              this.$message.error(
                `文件大小不能超过${(this.size / 1024).toFixed(2)}M`
              );
            } else {
