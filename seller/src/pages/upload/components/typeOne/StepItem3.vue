<template>
  <div class="stepGiftEdit">
    ...
    width:800px;margin:0 auto;margin-top: 30px;">
      <div>
        ...
        >
          ...
          <!-- <el-button type="primary" v-if="giftType == 0 && uploadTypeOne.validedOrders[0].gift"  class="setButton" @click="hanleweght()">批量修改重量</el-button> -->
        </el-radio-group>
      </div>
      <div>
        <a
          class="color-primary"
          href="javascript:;"
          @click="$refs.goodsNumSelectDialog.show(uploadTypeOne.orderList, uploadTypeOne.validedOrders);"
        >设置采购数量</a>
      </div>
    </div>
    ...
    <div class="chooseListBox">
      <div
        class="chooseItem"
        v-for="(item, i) in uploadTypeOne.validedOrders"
        :key="i"
      >
        <div class="title">
        ...
        >
          ...
          <div class="bottomTotal">
            ...
            <!-- <span class="flex1 center">快递总价：-- 元</span> -->
            <span class="flex1 center">商品总价：{{(item.gift.price * item.countNum).toFixed(2)}}元</span>
            <!-- <span class="flex1 center">单个包裹总价：--元</span> -->
            <!-- <span class="flex1 center">
                    总价：
                    <strong>￥--</strong>
                  </span> -->
          </div>
  ...
  methods: {
    ...
    hanleweght(index) {
      ...
      if(typeof index == 'number') {
        this.inx = index;
        this.weg_input = this.uploadTypeOne.validedOrders[index].gift.weight
        return false;
      }
      this.weg_input = '';
      this.inx = ''
    ...
    handleexit () {
      ...
      if(typeof this.inx == 'number') {
        this.uploadTypeOne.validedOrders[this.inx].gift.weight = val;
        return false;
      }
      ...
      this.uploadTypeOne.validedOrders.map(ele => {
        ele.gift.weight = val;
      });
    ...
    // 选择礼品
    selectGift(gift) {
      // console.log(this.uploadTypeOne.validedOrders);
      let validedOrders = this.uploadTypeOne.validedOrders;
    ...
    async addOrder() {
      console.log(
        this.uploadTypeOne.orderList,
        this.uploadTypeOne.validedOrders
      );
      // this.c
      ...
      let orderList = this.uploadTypeOne.orderList;
      let validedOrders = this.uploadTypeOne.validedOrders;

      let params = {
        type: this.uploadTypeOne.type,
        importId: this.uploadTypeOne.importId,
        sellerCartVo: [],
        remark: this.remark
      };
