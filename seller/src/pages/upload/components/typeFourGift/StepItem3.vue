  ...
  <div class="stepGiftEdit">
    ...
    width:800px;margin:0 auto;margin-top: 30px;">
      <div class="step1">
        ...
        >
          <el-radio :label="0">使用普通礼品
                <img src="@/assets/images/lp-icon.png" alt />
          </el-radio>
          <el-radio :label="1">使用信封礼品
                <img src="@/assets/images/xf-icon.png" alt />
          </el-radio>
        <!-- <el-button type="primary" class="setButton" v-if="giftType == 0 && validedOrders[0].gift" @click="hanleweght()">批量修改重量</el-button> -->
        </el-radio-group>
      </div>
      <div>
        <a
          class="color-primary"
          href="javascript:;"
          @click="$refs.goodsNumSelectDialog.show(orderList, validedOrders);"
        >设置采购数量</a>
      </div>
    </div>
        ...
        >
          ...
          <div class="bottomTotal">
            ...
            <span class="flex1 center">
              商品总价：{{(item.gift.price * item.countNum).toFixed(2)}}元
            </span>
  ...
  methods: {
    ...
    hanleweght(index) {
      ...
      if(typeof index == 'number') {
        this.inx = index;
        this.weg_input = this.validedOrders[index].gift.weight
        return false;
      }
      this.weg_input = '';
      this.inx = ''
    ...
    handleexit () {
      ...
      if(typeof this.inx == 'number') {
        this.validedOrders[this.inx].gift.weight = val;
        return false;
      }
    ...
    onChangeOrderList(o) {
      // 去主界面修改值
      this.$emit("change-change-orderslist", o.orderList);
      this.$emit(
        "change-change-orders",
        JSON.parse(JSON.stringify(o.validedOrders))
      );
    },
    // 选择礼品
    selectGift(gift) {
      // console.log(this.uploadTypeOne.validedOrders);
      let validedOrders = this.validedOrders;
    ...
    // 开始去下单，获取相关的金额。
    async addOrder() {
      // console.log(this.validedOrders);
