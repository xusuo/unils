<template>
  <div class="commonQuestion">
    ...
    <div class="work-order">
      如果您遇到我们未曾解答的问题请
      <span @click="$refs.wordOrderDialog.show();">提交工单>></span>
    </div>
          ...
          >
            ...
            <ul class="list">
              <li
                ...
                :key="j"
              >
                <span @click="switchQuestion(i, j)">{{chi.name}}</span>
              </li>
            </ul>
          </div>
        </div>
    ...
    <WordOrderDialog ref="wordOrderDialog" />
  </div>
</template>

<script>
import WordOrderDialog from "../components/WordOrderDialog";
export default {
  components: { WordOrderDialog },
  data() {
    return {
      questions: [
        {
  ...
  methods: {
    switchQuestion(i, j) {
      this.$router.push(
        `/userCenter/helpCenter/commonQuestionDetail?i=${i}&j=${j}`
      );
    }
  }
};
