<template>
  <div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <template v-if="scope.row.packageStatus == 1">待支付</template>
            <template v-if="scope.row.packageStatus == 2">已支付待发货</template>
            <template v-if="scope.row.packageStatus == 3">已发货</template>
            <template v-if="scope.row.packageStatus == 4">已完成</template>
          </template>
        </el-table-column>
        <el-table-column
          label="快递单号"
    ...
    <PackageDetailDialog ref="packageDetailDialog" />
  </div>
</template>
  ...
  methods: {
    async getTableData() {
      ...
      let res = await this.$http({
        url: "/t-operate/queryPackageList",
        method: "get",
        params
      });
      const { data } = res;
