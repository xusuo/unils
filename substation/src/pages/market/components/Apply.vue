<template>
  <div>
    ...
    >
      <div class="name">
        <!--  <span class="color-danger">{{balance}}</span> 元 -->
...
export default {
  ...
  methods: {
    ...
    async queryStationBalanceById(params) {
      // /t-sale/queryStationBalanceById
      let res = await this.$http({
        url: "/t-sale/querySaleBalanceById",
        method: "get",
        params
      });
      const { code, data } = res;
    ...
    async getTableData() {
      ...
      // console.log(params);
      let res = await this.$http({
        url: "/t-sale/queryWithdrawalsDataList",
        method: "get",
        params
      });
      const { data } = res;
