<template>
  <div>
        ...
        >
          ...
          >
            ...
            <template slot-scope="scope">
                <template>{{scope.row.fees + '元'}}</template>
              </template>
            </el-table-column>
            <el-table-column
              label="交易类型"
          ...
          <el-table-column
            ...
            >
              <template slot-scope="scope">
                <el-button
              size="small"
              @click="$refs.WithuserInfoDialog.show(scope.row);"
            >详情</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <WithuserInfoDialog ref="WithuserInfoDialog" />

  </div>
</template>

<script>
import WithuserInfoDialog from "./WithuserInfoDialog";

export default {
  components: { WithuserInfoDialog },
  data() {
    return {
      loading: false,
      tableData: [],
  ...
  methods: {
    async queryStationBalanceById(params) {
      // /t-agent/queryStationBalanceById
      let res = await this.$http({
        url: "/t-agent/queryAgentBalanceById",
        method: "get",
        params
      });
      const { code, data } = res;
  ...
  created() {
    this.queryStationBalanceById({
      stationId: this.userInfo.stationIds
    });
    this.handleSearch();
  }
};
